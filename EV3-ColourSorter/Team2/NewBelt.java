...
public class NewBelt {
	...
	public static void main(String[] args) {
		...
		// We take 5 readings for each block to be more confident on our colour decision
		float[] ColorSamples = new float[5];
		int i, angle = 0;

		// Whole program keeps running until the user presses the Enter button of the EV3 Smart Brick
		while (Button.ENTER.isUp()) {

			// ServoMotor object assumes it is at zero angle position when created, so
			// robot must be in it's starting configuration when starts.
			// Reposition hitting lever's servoMotor to 0 degrees when it offsets by errors (Correcting big errors)
			if (angle >= 20) {
			...
			// colorID 1.0 by the sensor that is BLACK colour (our conveyor belt colour)
			if ((ColorSamples[0] != -1) && (ColorSamples[0] != 7.0)
				...
				// Take 4 more Colour sample readings and store them in the array ColorSamples
	...
	// Used to drive the conveyor belt at a constant speed (feeding the Lego bricks)
	private static void driveBelt(int power) {
		beltMotor.setPower(power);
		beltMotor.forward();
	}

	// Used to move the lever that hits and sorts the bricks (direction flag: -1 is LEFT, 1 is RIGHT) 
	// This lever has to move very fast and with large acceleration to hit hard the bricks
	private static void hitBrick(int direction) {
		// Sets desired motor speed, in degrees per second. The maximum reliably velocity is 100 x battery voltage
		servoMotor.setSpeed(1700);
		
		// Sets the acceleration rate of this motor in degrees/sec/sec. The default value is 6000
	...
	// Examine if all 5 readings for each block are equal, meaning that we decided with
